...
class GradingEngine:
    ...
    def __init__(self):
        ...
        self._dao = dao_factory.submission()
    ...
    async def get_result(self, submission_id: str) -> Optional[Dict]:
        """获取批改结果"""
        row = await self._dao.get_by_id(submission_id)
        if not row:
            return None
    ...
    async def record_feedback(
        ...
        """记录批改反馈"""
        await self._dao.update(submission_id, {
            "feedback_rating": rating,
            "feedback_comment": comment
        })
        logger.info(f"Recorded feedback for submission: {submission_id}")

    async def list_by_assignment(self, assignment_id: str, page: int = 1, size: int = 20) -> Dict[str, Any]:
        """列出作业的所有提交"""
        rows = await self._dao.list_by_assignment(assignment_id, limit=size, offset=(page-1)*size)
        
        # 获取总数
        from shared.database import db_manager
        count_sql = "SELECT COUNT(*) as count FROM submissions WHERE assignment_id = ?"
        async with db_manager.connection.execute(count_sql, (assignment_id,)) as cursor:
    ...
    async def list_by_student(self, student_id: str, page: int = 1, size: int = 20) -> Dict[str, Any]:
        """列出学生的所有提交"""
        rows = await self._dao.list_by_student(student_id, limit=size, offset=(page-1)*size)
        
        # 获取总数
        from shared.database import db_manager
        count_sql = "SELECT COUNT(*) as count FROM submissions WHERE student_id = ?"
