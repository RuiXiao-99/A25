...
class QAEngine:
    ...
    def __init__(self):
        ...
        self._dao = dao_factory.qa_history()
        ...
        if student_id:
            try:
                await self._dao.insert({
                    ...
                    "question": question,
                    "answer": answer,
                    "knowledge_points": [k.get("knowledge_id") for k in knowledge[:3]],
    ...
    async def get_student_history(self, student_id: str, limit: int = 50) -> List[Dict]:
        """获取学生的问答历史（从数据库）"""
        rows = await self._dao.list_by_student(student_id, limit=limit)
        return [
            {
                "id": row["id"],
    ...
    async def feedback(self, qa_id: str, is_helpful: bool) -> bool:
        """
        对问答进行反馈
        ...
        """
        try:
            await self._dao.update(qa_id, {"is_helpful": is_helpful})
            return True
        except Exception as e:
